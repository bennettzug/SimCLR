...
def plot_tsne(embeddings,labels, epoch,output_dir, perplexity = 30, use_kmeans=False, n_clusters=10):
    ...
    if use_kmeans or len(np.unique(labels)) <=1:
        ...
        for i,label in enumerate(unique_labels):
            plt.scatter(embeddings_2d[cluster_labels == label, 0], embeddings_2d[cluster_labels == label, 1], color=colors[i], label=f'Class {label}',alpha=0.7,s=20)
        ...
        for i,label in enumerate(unique_labels):
            plt.scatter(embeddings_2d[labels == label, 0], embeddings_2d[labels == label, 1], color=colors[i], label=f'Class {label}',alpha=0.7,s=20)
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default=None, help="Path to the output directory (default: log_dir/tsne_plots)")
